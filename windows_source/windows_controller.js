const windows_initialize = require('./initialize');
const windows_get_devices = require('./list_devices');
...
async function init() {
    try {
        await windows_initialize();
        generateListOfDevices();
        setInterval(() => {
            generateListOfDevices();
        }, REFRESH_TIME);
    } catch (error) {
        error_handler("Error Initializing Windows dependencies!" + msg);
    }
}
...
function generateListOfDevices() {
    try {
        let device_list = [];
        for (device_ip of windows_get_devices()) {
