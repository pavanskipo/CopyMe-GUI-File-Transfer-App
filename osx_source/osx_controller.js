const osx_initialize = require('./initialize');
const osx_get_devices = require('./list_devices');
const error_handler = require('../errors/log_and_exit');
const http = require('../utilities/http_requests');

// Declarations
let LIST_OF_DEVICES = [];
const REFRESH_TIME = 15000;

function getListOfDevices() {
    return new Promise(async (resolve, reject) => {
        let device_details_list = [];
        for (let ip of LIST_OF_DEVICES) {
            let device = await http.httpGetDevice(ip);
            let device_details = {};
            if (device) {
                device_details = {
                    name: device.name,
                    ip: ip,
                    os: device.os,
                    os_image: device.os_image
                }
            } else {
                device_details = {
                    name: ip,
                    ip: ip,
                    os: 'NA',
                    os_image: '/images/default.png'
                }
            }
            device_details_list.push(device_details);
        }
        resolve(device_details_list);
    });    
}

function getSingleDeviceDetails(ip) {
    return new Promise(async (resolve, reject) => {
        let device_details = {};
        let device = await http.httpGetDevice(ip);
        if (device) {
            device_details = {
                name: device.name,
                ip: ip,
                os: device.os,
                os_image: device.os_image
            }
            resolve(device_details);
        } else {
            reject("device not supported");
        }
    })
}

async function init() {
    try {
        await osx_initialize();
        generateListOfDevices();
        setInterval(() => {
            generateListOfDevices();
        }, REFRESH_TIME);
    } catch (error) {
        error_handler("Error Initializing OSX dependencies!" + msg);
    }
}

function generateListOfDevices() {
    try {
        let device_list = [];
        for (device_ip of osx_get_devices()) {
            device_list.push(device_ip);
        }
        LIST_OF_DEVICES = [...device_list];
    } catch (error) {
        error_handler("error while getting the list of devices " + error);
    }
}

module.exports = {
    initialize: init,
    getListOfDevices: getListOfDevices,
    getSingleDeviceDetails: getSingleDeviceDetails
}